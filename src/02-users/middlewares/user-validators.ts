...
const userLoginInputValidator =
    body('login')
        ...
        //TODO написать тест на эту проверку (.matches(/^[a-zA-Z0-9_-]*$/))
        .matches(/^[a-zA-Z0-9_-]*$/)
        .withMessage('The "login" field must only contain letters (a-z, A-Z), numbers (0-9), underscores (_), and hyphens (-). Special characters, spaces, and other symbols are not allowed.')
        .trim()
